...
from common_line import append_jsonl, load_config, read_jsonl, resolve_config_path, resolve_optional_path, write_jsonl_atomic
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Global picker for WIN_YOLO_PTZ_20S")
    ...
    base_dir_raw = args.base_dir.strip() or str(conf.get("base_dir", "")).strip()
    if not base_dir_raw:
        raise SystemExit("base_dir is empty; set it in config or pass --base-dir")
    if args.base_dir.strip():
        base_dir = Path(base_dir_raw).resolve()
    else:
        base_dir = resolve_optional_path(conf, "base_dir")
        if base_dir is None:
            raise SystemExit("base_dir is empty; set it in config or pass --base-dir")
    event_queue = resolve_config_path(conf, "event_queue")
    route = conf.get("route", "win_yolo_ptz_20s")
