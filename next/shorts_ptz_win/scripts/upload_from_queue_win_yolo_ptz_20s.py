...
from common_line import clone_resolved_config, load_config, read_jsonl, resolve_config_path, resolve_optional_path, write_jsonl_atomic
...
def build_time_line(video_path: Path, decision_path: Path) -> str:
    ...
    return t.strftime("%Y-%m-%d %H:%M") + "\u9803 \u30e9\u30a4\u30d6\u914d\u4fe1\u4e2d\u306e\u4e00\u5834\u9762\u3067\u3059"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Upload from WIN_YOLO_PTZ_20S upload queue")
    ...
    queue_path = resolve_config_path(conf, "upload_queue")
    token_override = args.token.strip()
    if token_override:
        token_path = Path(token_override).resolve()
    else:
        token_conf = resolve_optional_path(conf, "youtube_token")
        token_path = token_conf if token_conf is not None else Path(DEFAULT_TOKEN_PATH)
    playlist_id = args.playlist or str(conf.get("playlist_id", DEFAULT_PLAYLIST_ID))
    ...
    skipped = 0
    for item in jobs:
        video_path = Path(item.get("video_path", "")).resolve()
        decision_path = Path(item.get("decision_path", "")).resolve()
        published_flag_path = Path(item.get("published_flag_path", "")).resolve()
        ...
        title = str(decision.get("title") or "Masao one scene").strip()
        description = build_description(decision, video_path, decision_path, conf)

        if args.dry_run:
            print(f"[DRY] title={console_safe(title)}")
            done += 1
            continue
