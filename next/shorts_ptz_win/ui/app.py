...
# WIN_YOLO_PTZ_20S のディレクトリ構造に合わせる
...
class App(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("WIN_YOLO_PTZ_20S パイプライン起動")
        ...
        self.publish_start = tk.StringVar(value=datetime.now(JST).strftime("%Y/%m/%d %H:00"))
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(actions, text="プロジェクトフォルダを開く", command=self.open_root_folder).pack(side="left", padx=4)
        row += 1
        ...
        manual_box = ttk.LabelFrame(frm, text="マニュアル候補選択 (Pick from list to queue)")
        ...
        ttk.Button(manual_box, text="候補読み込み", command=self.load_manual_candidates).grid(row=0, column=6, padx=6)
        ttk.Button(manual_box, text="選択したものをEnqueue", command=self.enqueue_selected_candidates).grid(row=0, column=7, padx=6)
    ...
    def reload_api_settings_from_config(self, log_warn: bool) -> None:
        ...
        if isinstance(env_file, str) and env_file.strip():
            # config からの相対パス対応 (DEFAULT_CONFIGの親基準とする)
            abs_env = (DEFAULT_CONFIG.parent / env_file.strip()).resolve()
            self.api_env_file.set(str(abs_env))
        elif log_warn:
        ...
        # base_dir も config にあれば上書き
        bd = conf.get("base_dir")
        if bd:
            self.base_dir.set(bd)
    ...
    def _event_queue_path(self) -> Path | None:
        ...
        if not q:
            ...
            return None
        # 相対パスの場合は config.json のあるディレクトリ基準
        p = Path(q)
        if not p.is_absolute():
            p = DEFAULT_CONFIG.parent / p
        return p.resolve()
    ...
    def enqueue_selected_candidates(self) -> None:
        ...
        route = str(conf.get("route", "win_yolo_ptz_20s"))
    ...
    def run_build(self) -> None:
        cmd = [
            "python",
            str(SCRIPTS / "build_candidates_win_yolo_ptz_20s.py"),
            "--config",
    ...
    def run_pick(self) -> None:
        ...
        cmd = [
            "python",
            str(SCRIPTS / "pick_candidates_win_yolo_ptz_20s.py"),
            "--config",
            self.config_path.get(),
            "--base-dir",
            self.base_dir.get(),
    ...
    def run_pipeline(self) -> None:
        cmd = ["python", str(SCRIPTS / "run_event_queue_pipeline_win_yolo_ptz_20s.py"), "--config", self.config_path.get()]
    ...
    def run_upload(self) -> None:
        cmd = ["python", str(SCRIPTS / "upload_from_queue_win_yolo_ptz_20s.py"), "--config", self.config_path.get()]
        self._run_cmd(cmd)
